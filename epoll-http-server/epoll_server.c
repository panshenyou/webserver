...
void http_request(const char* request, int cfd)
{
    ...
    if(S_ISDIR(st.st_mode)) {  		// 目录 
        ...
        const char * ret=get_file_type(".html");
        printf("文件类型：%s\n",ret);
        send_respond_head(cfd, 200, "OK", ret, -1);
        // 发送目录信息
        send_dir(cfd, file);
...
// 发送响应头
void send_respond_head(int cfd, int no, const char* desp, const char* type, long len)    //send_respond_head(cfd, 200, "OK", get_file_type(".html"), -1);
{
    char buf[1024] = {0};
    // 状态行
